...
There aware, however, that there are two distinct ways this feature can be used, and the developer should have a very clear view of which approach is being adopted. 
...
Suppose now that you want to have the ability to update certain paragraphs of the article on the fly.  This would mean that an efficient (re)rendering function / derendering function may be useful.  

In this case, including the renderer / derendering function inline would be wasteful, as it won't be able to be cached.  So it would be better to use the "traditional" approach mentioned earlier:
